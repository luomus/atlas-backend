      ...
      atlasDataDao.getGridAndBreedingdataForSpeciesAndAtlas(req.param('speciesId'), req.param('atlasId')).then((data) => {
        speciesDao.getById(req.param('speciesId')).then((species) => {
          ...
          if (req.param('type') === 'png') {
            ...
            mapService.getSpeciesMap(data, species[0], callback, 'png', req.param('scaling'), req.param('language'), req.param('atlasId'))
          } else {
            res.setHeader('Content-Type', 'image/svg+xml')
            res.send(mapService.getSpeciesMap(data, species[0], undefined, 'svg', req.param('scaling'), req.param('language'), req.param('atlasId')))
          }
        })
      })
    }
