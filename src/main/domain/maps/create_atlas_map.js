  ...
  function drawGrid(gridArray, svgImage) {
    ...
    svgGridArray.forEach((rect) => {
      const circlePropertyMap = {
        id: rect.id, class: 'gridCircle', cx: (rect.e), cy: (rect.n),
        fill: 'black', r: overlayCircleRadius, display: 'none',
      }
      const backgroundPropertyMap = {
        id: (rect.id + 'bg'), class: 'gridBackground',
        x: (rect.e - 0.5), y: (rect.n - 0.5), width: 1, height: 1, fill: 'lightgrey', display: 'none',
      }
